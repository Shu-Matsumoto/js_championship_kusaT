...
async function getTransferMessageWithSignature(rpc, walletPrivateKey, contractAddress, forwarderAddress, toAddress, tokenId) {
    ...
    const hokusaiInterface = new ethers_1.ethers.utils.Interface(ERC721WithRoyaltyMetaTx_json_1.default.abi);
...
async function getBurnMessageWithSignature(rpc, walletPrivateKey, contractAddress, forwarderAddress, tokenId) {
    const provider = new ethers_1.ethers.providers.JsonRpcProvider(rpc);
    const signer = new ethers_1.ethers.Wallet(walletPrivateKey);
    const { chainId } = await provider.getNetwork();
    const address = await signer.getAddress();
