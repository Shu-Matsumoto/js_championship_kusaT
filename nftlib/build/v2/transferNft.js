...
async function transferNft(baseUrl, network, apiKey, contractVer, contractId, messageWithSignature) {
...
async function main() {
    ...
    try {
        const messageWithSignature = await MetaTx_1.getTransferMessageWithSignature(RPC, walletPrivateKey, contractVer, contractAddress, forwarderAddress, toAddress, tokenId);
        const res = await transferNft(baseUrl, network, apiKey, contractVer, contractId, messageWithSignature);
        console.log(res);
    }
    catch (error) {
        console.log(error);
